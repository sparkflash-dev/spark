...
import { requestMetadataReady } from '../../app';
...
// Safe renderer for warning messages that may contain <a href="...">text</a> links.
// Only allows https:// and http:// hrefs to prevent javascript: XSS.
...
> {
			...
			promise: (async () => {
				...
				if (isString(selected)) {
					...
					try {
						...
						// Wait for the sidecar API to be ready (resolves once connected,
						// no polling needed — fixes #4150)
						const metadataFn = await Promise.race([
							requestMetadataReady,
							new Promise<never>((_, reject) =>
								setTimeout(
									() => reject(new Error('Metadata API timed out')),
									15000,
								),
							),
						]);

						metadata = await metadataFn({ selected, SourceType, auth });
	...
	private async onDrop(event: React.DragEvent<HTMLDivElement>) {
		...
		if (file != null) {
			await this.selectSource((file as any).path, 'File').promise;
		}
	}
		...
		return (
			<>
				...
				{this.state.warning != null && (
					...
					>
						<ModalText>
							{renderMessageWithLinks(this.state.warning.message)}
						</ModalText>
					</SmallModal>
				)}
