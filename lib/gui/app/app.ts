...
// Expose requestMetadata as a promise so callers can await it
// instead of polling. Resolves once the unprivileged sidecar connects.
let _resolveRequestMetadata!: (
	fn: (params: any) => Promise<SourceMetadata>,
) => void;
let _rejectRequestMetadata!: (err: Error) => void;

export const requestMetadataReady: Promise<
	(params: any) => Promise<SourceMetadata>
> = new Promise((resolve, reject) => {
	_resolveRequestMetadata = resolve;
	_rejectRequestMetadata = reject;
});

// Keep a synchronous reference for backwards compat with any remaining callers
export let requestMetadata: ((params: any) => Promise<SourceMetadata>) | undefined;
...
spawnChildAndConnect({
	...
	.then(({ emit, registerHandler }) => {
		...
		// build the requestMetadata function and resolve the promise
		const fn = async (params: any): Promise<SourceMetadata> => {
		...
		requestMetadata = fn;
		_resolveRequestMetadata(fn);

		registerHandler('drives', (data: any) => {
			try {
				setDrives(JSON.parse(data));
			} catch (e) {
				console.error('Failed to parse drives data:', e);
			}
		});
	})
	.catch((error: any) => {
		const err = new Error(`Failed to start the flasher process: ${error}`);
		_rejectRequestMetadata(err);
		throw err;
	});
