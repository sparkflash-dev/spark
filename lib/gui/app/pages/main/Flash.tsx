			...
			<>
				...
				>
					...
					<ProgressButton
						...
						cancel={() => this.setState({ showCancelConfirmation: true })}
				...
				{this.state.showCancelConfirmation && (
					<SmallModal
						width={400}
						title={i18next.t('flash.cancelTitle', 'Cancel flashing?')}
						cancel={() => this.setState({ showCancelConfirmation: false })}
						done={() => {
							this.setState({ showCancelConfirmation: false });
							imageWriter.cancel('cancel');
						}}
						action={i18next.t('flash.cancelConfirm', 'Yes, cancel')}
					>
						<Txt>{i18next.t('flash.cancelWarning', 'Cancelling mid-flash may leave the drive in an unusable state. Are you sure you want to cancel?')}</Txt>
					</SmallModal>
				)}
