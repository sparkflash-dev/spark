		...
		wss.on('connection', (ws) => {
			...
			const onSourceMetadata = async (params: any) => {
				...
				try {
					...
					emitFail(new Error('Invalid sourceMetadata params: not valid JSON'));
					return;
				}
				const { selected, SourceType, auth } = parsed;
				if (typeof selected !== 'string' && typeof selected !== 'object') {
					emitFail(new Error('Invalid sourceMetadata params: missing selected'));
					return;
				}
				try {
			...
			// Whitelist of allowed message types — reject anything else
			...
			ws.on('message', async (jsonData: any) => {
				...
				try {
					...
					console.error('Received malformed JSON message — ignoring');
					return;
				}
				if (typeof data.type !== 'string' || !ALLOWED_MESSAGE_TYPES.has(data.type)) {
					console.error(`Unknown or disallowed message type: ${data.type}`);
					return;
				}
				const handler = messagesHandler[data.type];
				if (handler) {
					try {
						await handler(data.payload);
					} catch (e) {
						console.error(`Error handling message type ${data.type}:`, e);
					}
				}
			});
